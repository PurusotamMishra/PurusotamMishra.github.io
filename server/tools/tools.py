...
from tools.utils.tracing import ExecutionMetrics, format_workflow_summary
from tools.schemas.models import SSEEvent
from tools.workflow_generator import stream_workflow
...
async def stream_workflow_events(
    ...
    Args:
        ...
        conversation_id: Optional conversation ID for session context
        
    Yields:
        SSE-formatted event strings
    """
    # Get or create session
    # print(f"[DEBUG] Session Management: Received conversation_id={conversation_id}")
    # session_id, session_data = session_manager.get_or_create_session(conversation_id)
    # print(f"[DEBUG] Session Management: Using session_id={session_id}")
    # print(f"[DEBUG] Session Management: Session has {len(session_data.previous_execution_results)} previous execution results")
    # print(f"[DEBUG] Session Management: Session has {len(session_data.previous_queries)} previous queries")
...
async def fetch_logs(user_query: str) -> list[dict]:
    """
    ...
    try:
        ...
        #     result.append(event)
        result = await stream_workflow_events(user_query)
        return result
    except Exception as e:
        raise e